...
(function (global) {
    ...
    function buildUI() {
        ...
        pip.innerHTML = `<video id="proctor-video" autoplay muted playsinline></video>
                         ...
                         <div id="proctor-status-bar">Status: OK</div>`;
    ...
    // ── Helper: detect if a frame is purely black or gray (blocked) ──────────
    function isFrameBlocked(canvas) {
        const ctx = canvas.getContext("2d");
        const w = canvas.width;
        const h = canvas.height;
        // Sample core area
        const imgData = ctx.getImageData(w/4, h/4, w/2, h/2);
        ...
        const finalAvg = brightnessSum / (data.length / 4);

        // Variance check to see if it's a solid color
        for (let i = 0; i < data.length; i += 4) {
            const r = data[i], g = data[i+1], b = data[i+2];
            const avg = (r + g + b) / 3;
            varianceSum += Math.abs(avg - finalAvg);
        }
        const variance = varianceSum / (data.length / 4);

        // If very dark (black) or very low variance (solid gray/color)
        return finalAvg < 8 || variance < 2;
    }

    // ── Send frame to Moondream backend ────────────────────────────────────
    async function analyzeFrame() {
        if (isAnalyzing || !isRunning || overlayVisible || !backendHealthy) return; 

        // Anti-Tamper 1: Check if stream is still active
        if (!stream || !stream.active) {
            handleIllegalActivity("camera disabled");
            return;
        }

        // Anti-Tamper 2: Check if PiP is hidden by student (via DevTools/CSS)
        const pipElement = document.getElementById("proctor-pip");
        if (pipElement) {
            const style = window.getComputedStyle(pipElement);
            if (style.display === "none" || style.visibility === "hidden" || parseFloat(style.opacity) < 0.1) {
                handleIllegalActivity("ui tampering");
                return;
            }
        }
        ...
        // Anti-Tamper 3: Check for blank/black/gray frames
        if (isFrameBlocked(canvasEl)) {
            const statusEl = document.getElementById("proctor-status-bar");
            if (statusEl) {
                statusEl.textContent = "Status: CAMERA BLOCKED / OFF";
                statusEl.className = "violation";
            }
            handleIllegalActivity("camera blocked");
            return;
        }
        ...
        try {
            ...
            // UI Feedback: Status Bar & Heartbeat
            const statusEl = document.getElementById("proctor-status-bar");
            const pipLabel = document.getElementById("proctor-pip-label");
            
            if (pipLabel) {
                pipLabel.classList.add("analyzing");
                setTimeout(() => pipLabel.classList.remove("analyzing"), 600);
            }

            let statusText = "OK";
            if (data.is_multi_person) statusText = "MULTI-PERSON DETECTED";
            else if (data.is_distracted) statusText = "LOOKING AWAY";
            
            if (statusEl) {
                statusEl.textContent = `Status: ${statusText}`;
                statusEl.className = (data.is_distracted || data.is_multi_person) ? "violation" : "";
                
                // Force Show: If hidden, show it
                const pip = document.getElementById("proctor-pip");
                if (pip && pip.style.display === "none") pip.style.display = "block";
            }

            console.debug(`[Proctor] [${new Date().toLocaleTimeString()}] AI Response:`, data.description);
